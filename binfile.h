...
private:
  ...
  Result InternalWrite(const T* val, size_t count)
  {
    ...
    {
			return Result::Failure(Format("Write failed, gle=%").gle().Str());
    }
		return (br == size) ? Result::Success() : Result::Failure();
  }
...
private:
  ...
  Result InternalWrite(const T* val, size_t count)
  {
    ...
    if(m_buffer.Size() < finalSize)
    {
      ...
      {
				return Result::Failure("Failed to allocate memory for buffered write operation");
      }
    }
