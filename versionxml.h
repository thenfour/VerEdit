...
public:
  ...
  Result Read(const string& fileName)
  {
    ...
    versionElement = doc->documentElement;
		...
		if(GetFirstElementByTagName(versionElement, "StringFileInfo", stringElement).Succeeded())
    {
    ...
    if(GetFirstElementByTagName(versionElement, "FixedFileInfo", fixedElement).Succeeded())
    {
    ...
    return Result(S_OK);
Error:
    return res.Prepend(Format("Error loading %; ").qs(fileName).Str());
  }
...
private:
  Result ProcessValueTag(MSXML::IXMLDOMElementPtr el, Arg::Type type, const string& source)
  {
    ...
    {
			return Result::Failure(Format("Com error: % %").s(e.ErrorMessage()).s(e.Description()).Str());
    }
